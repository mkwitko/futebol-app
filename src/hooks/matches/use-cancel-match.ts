...
import { useCancelMatch as useCancelMatchMutation } from "@/api/generated/hooks/matchesHooks";
import { MATCHES } from "@/api/modules/matches";

/** Cancela a pelada e invalida o detalhe + a lista de peladas do grupo em caso de sucesso. */
export function useCancelMatch(matchId: string) {
  const queryClient = useQueryClient();

  const mutation = useCancelMatchMutation({
    mutation: {
      onSuccess: (match) => {
